...
async def test_provider_client_uses_one_fixed_sync_contract_and_preserves_binding() -> None:
    ...
    assert transport.calls[0][0] == "https://provider.example/v1/device-channels/sync"
...
def test_provider_sync_schema_has_strict_request_and_response_defs() -> None:
    schema = json.loads(
        open("hub/contracts/schemas/channel/provider-sync.schema.json", encoding="utf-8").read()
    )
